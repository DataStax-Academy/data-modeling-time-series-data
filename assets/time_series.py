...
def populate_sources_by_group():
    print("""
INSERT INTO sources_by_group 
(group,source,characteristics,description)
VALUES ('House A','Termostate A1', 
        {'Model number':'EB-STATE5-01','Min temperature':'45F','Max temperature':'92F','Temperature sensitivity':'1F',
        'Min humidity':'5%','Max humidity':'95%','Humidity sensitivity':'1%'},
        'primary residence');
INSERT INTO sources_by_group 
(group,source,characteristics,description)
VALUES ('House A','Termostate A2', 
        {'Model number':'EB-STATE5-01','Min temperature':'45F','Max temperature':'92F','Temperature sensitivity':'1F',
        'Min humidity':'5%','Max humidity':'95%','Humidity sensitivity':'1%'},
        'primary residence');       
INSERT INTO sources_by_group 
(group,source,characteristics,description)
VALUES ('House A','Refrigerator A1', 
        {'Model number':'RF28R7551SG','Capacity':'15.6 cubic feet'},
        'primary residence');         
INSERT INTO sources_by_group 
(group,source,characteristics,description)
VALUES ('House A','Freezer A1', 
        {'Model number':'RF28R7551SG','Capacity':'8.3 cubic feet'},
        'primary residence');         
        
INSERT INTO sources_by_group 
(group,source,characteristics,description)
VALUES ('House B','Termostate B1', 
        {'Model number':'EB-STATE5-01','Min temperature':'45F','Max temperature':'92F','Temperature sensitivity':'1F',
        'Min humidity':'5%','Max humidity':'95%','Humidity sensitivity':'1%'},
        'vacation home');
INSERT INTO sources_by_group 
(group,source,characteristics,description)
VALUES ('House B','SmartSensor B1', 
        {'Model number':'EB-RSHM2PK-01'},
        'vacation home'); 
INSERT INTO sources_by_group 
(group,source,characteristics,description)
VALUES ('House B','SmartSensor B2', 
        {'Model number':'EB-RSHM2PK-01'},
        'vacation home');                                   
    """);
...
def populate_metrics():
    print("""
INSERT INTO metrics (bucket,metric,unit)
VALUES ('all','temperature','Fahrenheit');
INSERT INTO metrics (bucket,metric,unit)
VALUES ('all','humidity','% Relative Humidity');
INSERT INTO metrics (bucket,metric,unit)
VALUES ('all','occupancy','True/False or 1/0');                                   
    """);    
...
def populate_time_series(group,source,metric,min_value,max_value):
    ...
    for i in range(0,num_points_high):
        ...
        print("INSERT INTO series_by_source_high (group,source,timestamp,metric,value) VALUES (" + "'" + group + "'," + "'" + source + "'," + "'" + str(timestamp) + "'," + "'" + metric + "'," + str(point) + ");")
        print("INSERT INTO series_by_metric_high (group,source,timestamp,metric,value) VALUES (" + "'" + group + "'," + "'" + source + "'," + "'" + str(timestamp) + "'," + "'" + metric + "'," + str(point) + ");")
    # Populate low resolution tables
    for i in range(0,num_points_low):
        point = points_60m[i]     
        timestamp = latest_timestamp - i * resolution_60m       
        print("INSERT INTO series_by_source_low (group,year,source,timestamp,metric,value) VALUES (" + "'" + group + "'," + str(timestamp.year) + "," + "'" + source + "'," + "'" + str(latest_timestamp.replace(minute=0) - i * resolution_60m) + "'," + "'" + metric + "'," + str(point) + ");")
        print("INSERT INTO series_by_metric_low (group,year,source,timestamp,metric,value) VALUES (" + "'" + group + "'," + str(timestamp.year) + "," + "'" + source + "'," + "'" + str(latest_timestamp.replace(minute=0) - i * resolution_60m) + "'," + "'" + metric + "'," + str(point) + ");")
    ...
    for i in range(num_points_ignore,num_points_low,24):
        ...
        print("INSERT INTO statistics_by_source_metric (source,metric,date,min,max,median,mean,stdev) VALUES (" + "'" + source + "'," + "'" + metric + "'," + "'" + date + "'," + str(min) + "," + str(max) + "," + str(median) + "," + str(mean) + "," + str(std) + ");")
